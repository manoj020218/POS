...
import { assertSyncEventBranchesAccessible } from './sync-branch-access.js';
import type {
...
) => async (context: AccessContext, input: SyncPushRequest): Promise<SyncPushResult> => {
  const events = input.events.map(toCreateSyncEventInput(context.tenantId));
  assertRequestEventConsistency(events);
  await assertSyncEventBranchesAccessible(context, tenantCoreRepository, events);
